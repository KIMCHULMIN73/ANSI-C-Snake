...
void delete_foods(Board* board)
{
    ...
    if(board->foods != NULL)                                  // deleting all cells of foods linked list
    {
        CurP = board->foods;
        while(CurP!=NULL)
        {
...
enum Status move_snake(Board* board, enum Direction dir, int *x, int *y)
{
    ...
    head = next_move(board, dir);                             // Create a new head  
    if (head == NULL)
        return FAILURE;

    if (list_contains(head, board->snake))                    // Check for collision with snake itself
        return FAILURE;

    if (list_contains(head, board->enemy))                    // Check for collision with enemy
        return FAILURE;

    if (list_contains(head, board->foods))                    // Check for collision with foods
    {
        remove_from_list(head, &(board->foods));                // Remove the food collides with snake head from foods linked list
        snake_ate_food = 1;
        board->score+=10;
    }
    else 
...
enum Status move_enemy(Board* board, int *x, int *y)
{
    ...
    head = next_move_enemy(board);                            // Create a new head
    if (head == NULL)
        return FAILURE;

    if (list_contains(head, board->snake))                    // Check for collision with snake
        return FAILURE;
